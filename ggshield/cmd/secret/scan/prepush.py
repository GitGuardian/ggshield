...
OUTDATED_HOOK_MESSAGE = (
    "The installed pre-push hook did not pass its command-line"
    " arguments to ggshield. This can cause the hook to fail if the name of the remote"
    ' you are pushing to is not "origin".'
    """

This can happen if the hook has been created manually or by an old version of ggshield.

To fix it, either edit the hook manually or reinstall it with the following command:
...
def find_branch_start(commit: str, remote: str) -> str:
    ...
    output = git(
        ["rev-list", commit, "--topo-order", "--reverse", "--not", "--remotes", remote]
    )
