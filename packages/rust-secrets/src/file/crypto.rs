//! Device-local authenticated encryption for dotenv values.
//!
//! Values are sealed with XChaCha20-Poly1305 under a key derived from a
//! device-local master key (HKDF-SHA256, `info = "gitguardian-file-v1"`). The
//! extended 192-bit nonce means a fresh random nonce per value is safe with no
//! counter and no state to persist.
//!
//! The associated data is the variable's name followed by the envelope header,
//! so a ciphertext is bound both to the variable it was written for — copying
//! `API_KEY`'s value onto `DEBUG_FLAG` fails to authenticate instead of
//! silently moving a secret — and to the header a reader parsed it out of, so
//! the format version and scheme cannot be rewritten by anyone who can edit
//! the file.
//!
//! Plaintext is padded to a [`PAD_BLOCK`]-byte boundary before sealing. A
//! marker's length is otherwise `34 + len(plaintext) + 16`, which publishes the
//! exact byte length of every encrypted value in a committed `.env` — enough to
//! tell a 40-character token from a 20-character one.
...
/// Domain separation for the key derivation. Changing this invalidates every
/// value encrypted with the old one, so it is versioned.
const HKDF_INFO: &[u8] = b"gitguardian-file-v1";
/// Plaintext is padded up to a multiple of this before sealing, so an
/// envelope's length reveals only which 32-byte bucket the value falls in.
...
/// Length of a key id in the hex form used as a JSON object key.
const KEY_ID_HEX_LEN: usize = KEY_ID_LEN * 2;

/// Seals and opens dotenv values.
///
/// A trait so the envelope and dotenv layers can be exercised with a
/// deterministic cipher, and so a future scheme (a shared team key, a KMS)
/// slots in beside the device-local one.
...
/// The device's keys, newest first.
///
/// A set rather than a single key: rotating or importing a key must not need a
/// keyring format migration, and values encrypted with a retired key stay
/// readable.
...
// The keyset is key material; never let it reach a log or a panic message.
...
impl Keyset {
    ...
    /// Add every key of `other` that this keyset does not already hold.
    ///
    /// `self`'s current key stays current. This is the only safe way to write
    /// the keyring: the blob holds the whole keyset and a store replaces it
    /// wholesale, so anything not carried across is destroyed — and there is
    /// deliberately no recovery or export mechanism. Rotation and import will
    /// need exactly this too.
    ...
    /// Serialise for storage as one opaque keyring blob.
    ///
    /// Built by hand rather than through `serde_json::to_vec`: serde would copy
    /// each base64-encoded master key into an ordinary `String` and then into
    /// its own output buffer, and both are dropped unwiped. Only the key ids
    /// (which are not secret) and the punctuation are plain text here; every
    /// buffer the key material passes through is [`Zeroizing`].
    ///
    /// # Why the accumulator is sized up front
    ///
    /// `Zeroizing<String>` wipes the buffer it holds *when it is dropped*, and a
    /// `String` that grows moves: every reallocation after the first key was
    /// appended copies the key material into a fresh allocation and frees the old
    /// one untouched, so the wipe at the end covers only the last of them. The
    /// exact capacity is reserved before anything is written, so the buffer never
    /// moves and there is only ever one allocation to wipe. Same reason
    /// [`ENCODED_KEY_LEN`] is a constant rather than measured: an under-estimate
    /// silently reintroduces the reallocation.
    pub(crate) fn to_bytes(&self) -> Result<Zeroizing<Vec<u8>>> {
        ...
        for (index, (id, key)) in self.keys.iter().enumerate() {
            ...
            json.push_str(r#"":""#);
            // base64url-nopad: no character JSON would have to escape, which
            // is also what lets `from_bytes` borrow instead of copying.
            let encoded = Zeroizing::new(URL_SAFE_NO_PAD.encode(key.as_slice()));
            json.push_str(&encoded);
            // `encoded` is wiped by `Zeroizing`'s `Drop` here, over its whole
            // capacity. (`String::clear` would not: it only sets the length.)
            json.push('"');
        }
        json.push_str("}}");
        debug_assert_eq!(
    ...
    fn stored_len(&self) -> usize {
        ...
        // `"<id>":"<key>"` per key, plus a comma between consecutive ones.
        let per_key = KEY_ID_HEX_LEN + ENCODED_KEY_LEN + r#""":"""#.len();
        framing + self.keys.len() * per_key + self.keys.len().saturating_sub(1)
    }

    /// Parse a keyring blob written by [`Keyset::to_bytes`].
    ///
    /// The stored form borrows its strings straight out of `bytes` — which the
    /// caller keeps in a [`Zeroizing`] buffer — so no owned copy of a master key
    /// is made on the way in. A blob containing JSON escapes cannot be borrowed
    /// from and is reported as unreadable; nothing this crate writes has any.
...
impl Cipher for Keyset {
    fn encrypt(&self, key_name: &str, plaintext: &[u8]) -> Result<Envelope> {
        ...
        // The envelope is built first so its header can be authenticated; only
        // the ciphertext is filled in afterwards.
    ...
    fn decrypt(&self, key_name: &str, envelope: &Envelope) -> Result<Zeroizing<Vec<u8>>> {
        ...
        // Deliberately vague: a MAC failure must not say which of the value,
        // the variable name or the key was wrong, and must never echo bytes.
...
/// Associated data for a value: its variable name, then the envelope header.
///
/// Concatenated with no separator, which is unambiguous only because the header
/// is a fixed [`HEADER_LEN`] bytes and comes last.
...
///
/// Always adds between 1 and `PAD_BLOCK` bytes, each holding the number of
/// bytes added, so the padding is never ambiguous and the sealed length is
...
///
/// Only reached after the AEAD has authenticated the bytes, so a bad length or
/// a bad padding byte means whoever wrote this held the key but not the format.
...
/// The keyset as stored: one JSON blob, keys base64url-encoded.
///
/// Deserialised by borrowing from the caller's buffer, never by owning: an
/// owned `String` holding a master key would be dropped without being wiped.
/// Serialising is done by hand in [`Keyset::to_bytes`] for the same reason.
#[derive(Deserialize)]
...
pub(crate) mod test_cipher {
    //! A deterministic stand-in for [`Keyset`], so the envelope, dotenv and
    //! file layers can be tested without the OS keyring or random nonces.

    use super::*;

    /// Scheme byte used by the test cipher. Reserved as far as the real
    /// keyset is concerned, which is the point: a value written by this cipher
    /// must be rejected by [`Keyset`] as an unsupported scheme.
    ...
    impl TestCipher {
        ...
        /// A byte-for-byte reversible transform plus a trivial "tag" over the
        /// name — enough to exercise AAD binding and tamper detection.
    ...
    impl Cipher for TestCipher {
        fn encrypt(&self, key_name: &str, plaintext: &[u8]) -> Result<Envelope> {
            ...
            // A 16-byte "tag", so envelopes are the same shape as real ones.
...
mod tests {
    ...
    /// Finding 5: the primitive every keyring write goes through must add keys,
    /// never replace them — a replaced key is an unreadable value with no way
    /// back.
    #[test]
    fn absorbing_a_keyset_keeps_both_keys_readable() {
        ...
        // Before absorbing, their value is unreadable with my keyset.
    ...
    /// Finding 15: a marker must not publish its plaintext's exact length.
    #[test]
    fn the_envelope_length_reveals_only_a_block_bucket() {
        let keyset = Keyset::generate().unwrap();
        let mut lengths = std::collections::BTreeSet::new();
        // Every plaintext length inside one block must seal to one length, so a
        // 20-byte key id and a 31-byte token are indistinguishable on disk.
        ...
        // A value that crosses into the next block moves to the next bucket:
        // the length is coarse, not hidden altogether.
    ...
    fn invalid_padding_is_rejected_without_echoing_the_bytes() {
        ...
        assert!(unpad(&[]).is_err());
        // A padding byte that does not match its run.
        let mut bad = vec![0_u8; PAD_BLOCK];
        bad[PAD_BLOCK - 1] = 4;
        assert!(unpad(&bad).is_err());
        // A padding length bigger than a block.
    ...
    /// Finding 12: the header is parsed from file bytes before any crypto
    /// happens, so it has to be authenticated. Rewriting the scheme or the
    /// format version must not verify.
    #[test]
    fn the_envelope_header_is_authenticated() {
        let keyset = Keyset::generate().unwrap();
        let envelope = keyset.encrypt("API_KEY", FAKE_VALUE.as_bytes()).unwrap();

        // A relabelled format version fails the MAC rather than decrypting.
        // (`classify` rejects an unknown version earlier; this is the guarantee
        // that a *future* version cannot be downgraded to this one.)
        ...
        // A flipped nonce fails too — any header byte does.
    ...
    /// Finding 4: and the **key id** specifically, which the test above claimed
    /// to cover while mutating the nonce.
    ///
    /// Rewriting the id normally selects a different key, so a failure proves
    /// nothing — it would fail for the wrong reason. Two ids over one master key
    /// is the only arrangement that isolates the id: the derived key is
    /// identical either way, so if the id were not in the associated data the
    /// relabelled envelope would decrypt perfectly.
    #[test]
    fn the_key_id_is_part_of_what_the_tag_covers() {
        ...
        // Sanity: the two keys really are interchangeable as far as the cipher
        // is concerned, so nothing but the AAD can reject this.
    ...
    /// Finding 3/2: the blob is built in one allocation that is wiped on drop.
    ///
    /// `Zeroizing<String>` wipes the buffer it holds *at drop time*, so a
    /// `String` that grows leaves every earlier allocation — each holding the
    /// key material appended so far — freed and untouched. The guarantee is
    /// therefore "the accumulator never moves", which is what the debug
    /// assertion inside `to_bytes` checks and what this exercises: with three
    /// keys, a `String::new()` accumulator has to reallocate several times.
    ///
    /// The exact-length assertion is the other half: `stored_len` under-counting
    /// by even one byte puts the reallocation straight back.
    #[test]
    fn the_keyring_blob_is_built_without_reallocating_the_accumulator() {
        ...
        // And it is still the blob `from_bytes` reads.
        assert_eq!(Keyset::from_bytes(&bytes).unwrap().keys.len(), 3);
    }

    /// Finding 12: the AAD must actually contain the header bytes, and the
    /// variable name must still be distinguishable from them.
    #[test]
    ...
    /// Finding 3: no base64-encoded master key may be left in an ordinary heap
    /// allocation on the way to or from the keyring blob.
    #[test]
    fn the_stored_form_holds_no_owned_copy_of_a_key() {
        ...
        for encoded in stored.keys.values() {
            // Borrowed straight out of the caller's Zeroizing buffer: the
            // pointer is inside it, so there is no second copy to leak.
