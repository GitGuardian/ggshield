...
def test_agent_default_raw_history_sources_is_empty() -> None:
    assert _FakeAgent().raw_history_sources == []


def test_agent_iter_raw_history_events_loops_each_source(tmp_path: Path) -> None:
    """Each source is invoked with the agent's config_folder so source_path is relative to it."""
    from ggshield.verticals.ai.raw_history import JSONLHistorySource, RawHistoryEvent
    ...
    class A(_FakeAgent):
        raw_history_sources = [S1(), S2()]

        @property
        def config_folder(self) -> Path:
            return cfg

    events = list(A().iter_raw_history_events())
    assert events == [
        RawHistoryEvent(
            agent_name="fake",
            source_kind="k1",
