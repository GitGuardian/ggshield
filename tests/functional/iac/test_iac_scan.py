...
def test_iac_scan(tmp_path: Path) -> None:
...
def test_iac_scan_only_tracked_iac(tmp_path: Path) -> None:
...
def test_iac_scan_ignore_path(tmp_path: Path, ignored_paths: List[str]) -> None:
...
# TODO: test --ignore-policy and --minimum-severity once pygitguardian is updated
