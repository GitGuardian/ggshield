...
@pytest.mark.parametrize(
    ...
    (
        ...
        ("marshmallow", 0),
        # numba 0.52.0 declares Requires-Python >=3.6,<3.9, which no supported
        # interpreter satisfies. It must still download and scan: this guards #458,
        # where `pip download` refused packages incompatible with the running
        # interpreter.
        ("numba==0.52.0", 0),
    ),
)
