...
class TestScan(unittest.TestCase):
    ...
    def test_scan_simple_secret(self):
        patch = (
            "diff --git a/test.txt b/test.txt\n"
            "new file mode 100644\n"
            "index 0000000..b80e3df\n"
        ...
        with use_cassette(
            "secrets_shield/tests/cassettes/test_scan_simple_secret.yaml"
        ):
            results = asyncio.get_event_loop().run_until_complete(c.scan())
    ...
    def test_scan_multiple_secrets(self):
        ...
        with use_cassette(
            "secrets_shield/tests/cassettes/test_scan_multiple_secrets.yaml"
        ):
            results = asyncio.get_event_loop().run_until_complete(c.scan())
            assert process_scan_result(results) == 1
        ...
        self.assertEqual(result["has_leak"], True)
        self.assertEqual(result["error"], False)
        self.assertEqual(len(result["scan"]["secrets"][0]["matches"]), 2)
