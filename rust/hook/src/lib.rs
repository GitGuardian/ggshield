...
fn scan(config: &config::Config, stdin_content: &str) -> Result<Emission, Error> {
    ...
    // Ahead of anything this run stores: declining hands this same stdin to
    // `ggshield-py`, and a debounce entry already written would make it exit 0
    // with no verdict at all — an allow nobody decided. `debounce::store` runs
    // only once a verdict exists, so returning here leaves nothing behind.
    ...
    let result = match verdict {
        Verdict::Allowed => HookResult::allow(payload),
...
mod tests {
    ...
    fn only_a_pre_tool_use_on_an_mcp_tool_is_activity() {
        ...
        for event in [
            ...
            let emission = scan(&config, &event.to_string()).expect("settled");
            let code = match emission {
                Emission::Stdout(_, code) | Emission::Stderr(_, code) | Emission::Silent(code) => {
                    code
                }
            };
