...
impl Agent {
    ...
    pub fn mcp_activity(self, data: &Value, inventory: &Inventory) -> McpActivity {
        ...
        let (server, tool) = match self {
            ...
            // Dialect not ported yet. Junie matches its Python adapter exactly,
            // which reports the whole name because its prefix scheme is
            // unverified. Kiro ("@server/tool" and "mcp_<server>_<tool>") and
            // Vibe ("<configuration>_<tool>") do resolve a server in Python, so
            // these two report less than Python does until they are ported; the
            // API resolves what it can from the tool alone and counts the rest
            // in mcp_activity_lookup_failure_total.
            Agent::Junie | Agent::Kiro | Agent::Vibe => (String::new(), raw_tool_name.clone()),
        };
        McpActivity {
            server,
            tool,
