...
use crate::commands::shared::{Scope, confirm_existing_fields, field_count, write_path};
...
pub(crate) fn execute(args: Args) -> Result<()> {
    ...
    // Only after the write returned: a source removed before the values are
    // safely stored is a source removed for nothing.
    if let Some(input) = args.input.as_deref() {
