...
/// Approve this directory's dotenv file for `ggshield secret activate`.
///
...
pub(crate) struct Args {
    ...
    #[arg(long)]
    path: Option<String>,
    /// Which file to approve, git-config style (default: project).
    #[arg(long, value_enum)]
    scope: Option<Scope>,
...
pub(crate) fn execute(args: Args) -> Result<()> {
    if args.list {
        ...
        if paths.is_empty() {
            eprintln!("no dotenv files are trusted for `ggshield secret activate`");
            return Ok(());
        }
        for path in paths {
    ...
    // Deliberately not a prompt. The user is already running a command that
    // says what it does, and a y/n here would train them to answer it without
    // reading — which is the whole failure mode this gate exists to prevent.
    if !path.exists() {
        bail!(
    ...
    if trust::trust(path)? {
        eprintln!(
            "{} is now trusted; `ggshield secret activate` will load it. Any edit revokes this",
            path.display()
        );
    } else {
