...
//!
//! Four scopes, most specific winning variable by variable:
//!
//! | scope   | file                                          |
//! |---------|-----------------------------------------------|
//! | system  | `/etc/gitguardian/secrets.env`                |
//! | user    | `<config dir>/gitguardian/secrets.env`        |
//! | repo    | `<common git dir>/gitguardian/secrets.env`    |
//! | project | `./.env`, or whatever `--path` names           |
//!
//! # What the system scope can hold
//!
//! Every local user can read it, and the key that seals a value is in *one*
//! user's login keyring, so an encrypted value written there is unreadable by
//! everyone else on the machine — the failure is loud, but it is still a
//! failure. It is for values a machine's administrator sets for everyone:
//! plaintext configuration, and secret *paths* for the remote providers. It
//! becomes a place for sealed values once a key can be shared between
//! recipients, not before.
//!
...
/// The system-scope file, shared by every user of this machine.
///
/// `/etc/gitguardian/secrets.env` on Unix, and the machine-wide program-data
/// folder on Windows. Never created by this crate: writing it needs the
/// privileges its directory demands, which is what makes it the
/// administrator's file rather than another one of the user's.
...
pub(crate) struct FileBackend {
    ...
    /// The system-scope file, resolved once.
...
impl FileBackend {
    ...
    ///
    /// What `--scope` reads: "show me what this file sets" is a different
    /// question from "what would a command see here", and answering the second
    /// when the first was asked is how you end up editing the wrong file.
...
mod tests {
        ...
        fn new() -> Self {
            ...
            Fixture {
                backend: FileBackend {
                    ...
                    // A real /etc file would make these tests depend on the
                    // machine they run on.
    ...
    /// What `get --scope` asks: what does *this file* set, not what would a
    /// command see here.
    #[test]
