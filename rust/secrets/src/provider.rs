...
///
/// Most variants are backed by a bundled provider definition (see
/// [`crate::providers`]); [`Provider::ALL`] lists every supported backend.
...
pub enum Provider {
    /// Dotenv files encrypted with a device-local key. Has no definition: it
    /// makes no HTTP calls and has no base URL or token.
    File,
    Onepassword,
    Vault,
}
...
impl Provider {
    ...
    /// The canonical lowercase name (matches the definition's `name`).
    ...
    /// The bundled definition for this provider, if it has one.
    ///
    ...
    pub(crate) fn definition(self) -> Option<&'static ProviderDef> {
        match self {
            Provider::File => None,
            Provider::Onepassword => Some(&crate::providers::ONEPASSWORD),
