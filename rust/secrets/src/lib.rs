...
//! ```
//!
//! Providers are declared as static data in `providers.rs` and exposed as the
//! [`Provider`] enum; anything a declaration can't express (request signing,
//! multi-step token exchange) is a coded `auth` strategy referenced by name. Resolved
