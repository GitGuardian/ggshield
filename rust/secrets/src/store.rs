...
impl SecretStore {
    ...
    ///
    /// `file` provider only; the remote providers have one document per secret
...
mod tests {
    ...
    fn onepassword_store(base_url: String) -> (SecretStore, TestTokenFile) {
        ...
        let mut store = SecretStore::builder(Provider::Onepassword)
            ...
            .build()
            .unwrap();
        // Definitions are `&'static`, so a test that needs one pointing at a
        // mock server leaks a copy: a handful of bytes per test, and it keeps
        // the shipped type free of an owned-string case nothing else needs.
        store.http_mut().definition = Box::leak(Box::new(ProviderDef {
            base_url: leak(base_url),
            auth: Auth::Token {
                token_env: "GG_TEST_UNSET_CONNECT_TOKEN",
                token_file_env: None,
                token_file: Some(leak(token_file.0.to_string_lossy().into_owned())),
                header: "Authorization",
                scheme: Some("Bearer"),
            },
            endpoints: crate::providers::ONEPASSWORD.endpoints,
            name: crate::providers::ONEPASSWORD.name,
            description: crate::providers::ONEPASSWORD.description,
        }));
        (store, token_file)
    }
    ...
    fn parses_the_onepassword_connect_provider() {
        let provider = Provider::Onepassword.definition().unwrap();
        ...
        assert_eq!(
            provider.endpoint("write_secret").unwrap().method,
            Method::Put
        );
    }
