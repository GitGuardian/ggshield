...
/// Importing merges into the target the way `set` does, so a dotenv the user
/// maintains by hand keeps its comments and its order.
#[test]
...
/// `--scope` answers "what does this file set", the way `git config --global`
/// does, instead of what a command running here would see.
#[test]
fn get_scope_reads_one_file_rather_than_the_merge() {
    let workspace = Workspace::new();
    workspace.set(
        &["set", "--provider", "file", "--scope", "user", "SHARED"],
        "fake-user-value",
    );
    workspace.set(
        &["set", "--provider", "file", "--scope", "project", "SHARED"],
        "fake-project-value",
    );
    ...
    let scoped =
        stdout(&workspace.run(&["get", "--provider", "file", "--scope", "user", "--expose"]));
...
/// The conversion an existing project needs: a plaintext `.env` becomes the
/// repository's sealed store, and the file it came from is gone.
#[test]
